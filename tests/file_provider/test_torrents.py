...
class FakeRpc:
    def __init__(
        ...
        *,
        ...
        # When set, aria2 reports the download as still in progress so the
        # global-storage-quota path (which only pauses active downloads with
        # bytes still to fetch) is exercised. A completed torrent occupies its
        # bytes on disk and so is accounted for by the quota scan instead.
        ...
        self.files = [
            {
                ...
                "length": "4",
                "completedLength": "0" if in_progress else "4",
                "selected": "true",
            },
            {
                "index": "2",
                ...
                "length": "5",
                "completedLength": "0" if in_progress else "5",
                "selected": "true",
            },
        ]
    ...
    def call(self, method: str, params=None):
        ...
        if method == "aria2.tellStatus":
            return {
                "gid": params[0],
                "status": "active" if self.in_progress else "complete",
                "totalLength": "9",
                "completedLength": "0" if self.in_progress else "9",
                "downloadSpeed": "1024" if self.in_progress else "0",
                "uploadSpeed": "0",
                "errorCode": "0",
                "errorMessage": "",
                "infoHash": "abc123",
...
def test_global_storage_limit_pauses_new_download(tmp_path: Path) -> None:
    ...
    manager = TorrentManager(db, root, rpc=FakeRpc(root, in_progress=True), quota=quota)
    manager.start()
